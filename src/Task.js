...
function Task() {
  ...
  const setDtRangeNext = () => {
    ...
    if (getSelectedValue.value === 'M') {      
      firstDay = moment(firstDay).add(1,'months').startOf('month').format('MM/DD/YYYY');
      lastDay = moment(firstDay).endOf('month').format('MM/DD/YYYY');
      timeInterval = 30
      setSDate(firstDay);
      setEDate(lastDay);
      ...
      const date1 = new Date(moment(firstDay).format('MM/DD/YYYY'));
        const date2 = new Date(moment(lastDay).format('MM/DD/YYYY'));
        timeInterval = moment.duration(moment(date2).diff(moment(date1))).asDays()+1;
        firstDay = moment(lastDay).add(1, 'd').format('MM/DD/YYYY')
        lastDay = moment(lastDay).add(timeInterval, 'd').format('MM/DD/YYYY');
        setSDate(firstDay);
        setEDate(lastDay);
  ...
  const setDtRangePrev = () => {
    ...
    if (getSelectedValue.value === 'M') {
      timeInterval = 30
      firstDay = moment(firstDay).subtract(1,'months').startOf('month').format('MM/DD/YYYY');
      lastDay = moment(firstDay).endOf('month').format('MM/DD/YYYY');
      timeInterval = 30
      setSDate(firstDay);
      setEDate(lastDay);
